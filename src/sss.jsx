...
const PrintPreview = ({ html, csv, pdfBytes, title, onClose }) => {
  ...
  const ensureHtml2Pdf = () => new Promise((resolve, reject) => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => reject(new Error("Failed to load PDF export library.")), { once:true });
      return;
    }
    const s = document.createElement("script");
    s.id = "html2pdf-cdn-script";
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      if (!doc?.body) throw new Error("Preview is not ready yet.");
      ...
      exportRoot.style.width = (doc.documentElement.scrollWidth || frame.clientWidth || 1120) + "px";

      Array.from(doc.head.querySelectorAll("style, link[rel='stylesheet']")).forEach((node) => {
      ...
      const width = exportRoot.scrollWidth || 1120;
      const height = exportRoot.scrollHeight || doc.body.scrollHeight || 1600;
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:2000, background:"rgba(0,0,0,.88)", display:"flex", flexDirection:"column", backdropFilter:"blur(6px)" }}>
      <div style={{ background:"#0e1118", borderBottom:"1px solid rgba(255,255,255,.1)", padding:"11px 18px", display:"flex", alignItems:"center", gap:10, flexShrink:0, flexWrap:"wrap" }}>
        ...
        {tab === "preview" && (<>
          {pdfBytes && <button onClick={handleDownloadPDF} style={btnStyle("rgba(34,197,94,.15)","#22c55e","rgba(34,197,94,.35)")}>{preparingPdf ? "Preparing PDF..." : "⬇ Download PDF"}</button>}
