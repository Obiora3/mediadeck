...
const loadAppUserFromSupabase = async (authUser) => {
  ...
  return {
    ...
    agencyCode,
    agencyAddress,
    signatureDataUrl: authUser.user_metadata?.signature_data_url || "",
...
const buildMPOHTML = (mpo) => {
  const {
    ...
    agencyAddress, signedBy, signedTitle, signedSignature, preparedBy, preparedContact, preparedTitle, preparedSignature,
...
const MPOPage = ({ vendors, clients, campaigns, rates, mpos, setMpos, user, appSettings }) => {
  ...
  const openPreview = (mpo) => {
    const safeMpo = sanitizeMPOForExport({ ...mpo, preparedSignature: mpo?.preparedSignature || user?.signatureDataUrl || "", signedSignature: mpo?.signedSignature || "" });
...
const SettingsPage = ({ user, onUserUpdate, onLogout, appSettings, setAppSettings, vendors, clients, campaigns, rates, mpos, members, setMembers, notifications, unreadNotifications, onMarkNotificationRead, onMarkAllNotificationsRead }) => {
  ...
  const handleSignatureUpload = async (file) => {
    ...
    try {
      ...
      setToast({ msg: "Signature uploaded. Save profile to keep it permanently.", type: "success" });
    } catch (error) {
  ...
  }, [user?.id, user?.name, user?.title, user?.email, user?.phone, user?.agency, user?.agencyAddress, user?.signatureDataUrl]);
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10, padding: "12px 14px", borderRadius: 10, background: "var(--bg3)", border: "1px solid var(--border)" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        ...
                        {pf.signatureDataUrl ? <Btn variant="danger" size="sm" onClick={() => { setPf(p => ({ ...p, signatureDataUrl: "" })); onUserUpdate({ ...user, signatureDataUrl: "" }); }}>Remove</Btn> : null}
                      </div>
                    </div>
