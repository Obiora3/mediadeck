...
const uploadMpoAttachmentAndGetUrl = async ({ agencyId, mpoId, kind, file }) => {
  const resolvedAgencyId = await resolveAgencyIdForCurrentUser({ agencyId });
  if (!resolvedAgencyId) throw new Error("No agency found for this workspace.");
  if (!mpoId) throw new Error("No MPO found for this upload.");
  if (!file) throw new Error("No file selected.");
  const safeName = sanitizeAttachmentFileName(file.name || "file");
  const path = `${resolvedAgencyId}/${mpoId}/${kind}/${Date.now()}_${safeName}`;
...
const resolveAgencyIdForCurrentUser = async (fallback = {}) => {
  if (fallback?.agencyId) return fallback.agencyId;
  ...
  try {
    ...
    console.error("Failed to get current auth user while resolving agency:", error);
  }

  const targetId = authUser?.id || fallback?.id || null;
  const targetEmail = authUser?.email || fallback?.email || null;
  ...
  if (targetId) {
    ...
    if (!profileByIdError && profileById?.agency_id) return profileById.agency_id;
  }
  ...
  if (targetEmail) {
    ...
    if (!profileByEmailError && profileByEmail?.agency_id) return profileByEmail.agency_id;
  }

  if (authUser) {
    try {
      const ensured = await ensureAgencyForUser(authUser);
      if (ensured) return ensured;
    } catch (error) {
      console.error("Failed to ensure agency while resolving agency:", error);
    }
    ...
    if (refreshedProfile?.agency_id) return refreshedProfile.agency_id;
  }
...
const createClientInSupabase = async (agencyId, userId, form) => {
  ...
  const { data, error } = await supabase
    .from("clients")
    .insert([{ agency_id: resolvedAgencyId, created_by: userId, name: form.name.trim(), industry: form.industry || null, contact: form.contact || null, email: form.email || null, phone: form.phone || null, address: form.address || null, brands: form.brands || null, is_archived: false }])
    .select()
    .single();
  if (error) throw error;
...
const createNotificationForUserInSupabase = async ({ agencyId, recipientUserId, category = "workspace", title, message = "", recordType = null, recordId = null, linkPage = "dashboard", actor = null, metadata = {} }) => {
  ...
  if (error) {
    console.error("Notifications insert error (single recipient):", error);
    throw error;
  }
  return mapNotificationFromSupabase(data);
};
const createNotificationsForRolesInSupabase = async ({ agencyId, roles = [], excludeUserId = null, category = "workspace", title, message = "", recordType = null, recordId = null, linkPage = "dashboard", actor = null, metadata = {} }) => {
  ...
  if (error) {
    console.error("Notifications insert error (roles):", error);
    throw error;
  }
  return (data || []).map(mapNotificationFromSupabase);
};
...
const getThemeStorageKey = (userId = null) => `msp_theme_${userId || "guest"}`;
const getDefaultTheme = (userId = null) => {
  const saved = store.get(getThemeStorageKey(userId), null);
  if (saved === "light" || saved === "dark") return saved;
  if (!userId) {
    const legacy = store.get("msp_theme", null);
...
const Card = ({ children, style, glow, hoverable = false }) => {
  const baseBorder = glow ? "rgba(240,165,0,.25)" : "var(--border)";
  const baseShadow = glow ? "0 0 28px rgba(240,165,0,.07)" : "0 2px 12px rgba(0,0,0,.3)";
  return (
    <div
      style={{
        background: "var(--bg2)",
        ...
        transition: hoverable ? "transform .18s ease, box-shadow .18s ease, border-color .18s ease" : undefined,
        ...style,
      }}
      onMouseEnter={hoverable ? (e) => {
        e.currentTarget.style.transform = "translateY(-4px)";
        e.currentTarget.style.boxShadow = glow ? "0 18px 38px rgba(240,165,0,.12)" : "0 18px 38px rgba(0,0,0,.32)";
        e.currentTarget.style.borderColor = "rgba(240,165,0,.32)";
      } : undefined}
      ...
      {filtered.length === 0 ? <Card><Empty icon="📢" title="No campaigns found" sub="Create your first campaign" /></Card> :
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {filtered.map(c => {
            const cl = clients.find(x => x.id === c.clientId);
            return (
              <Card key={c.id} hoverable style={{ padding: "14px 18px" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800, fontSize: 11 }}>
            <thead><tr style={{ background: "var(--bg3)" }}>
              {["Vendor","Programme","Time Belt","Type","Dur","Rate/Spot","Disc%","Comm%","Net Rate",""].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontSize: 9, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".07em", color: "var(--text3)", borderBottom: "1px solid var(--border)", whiteSpace: "nowrap" }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {filtered.map((r, i) => {
                const vn  = vendors.find(v => v.id === r.vendorId)?.name || "—";
                ...
                return (
                  <tr key={r.id} style={{ borderBottom: "1px solid var(--border)", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,.01)" }}
                    ...
                    <td style={{ padding: "9px 10px", fontWeight: 600, fontSize: 11 }}>{vn}</td>
                    <td style={{ padding: "9px 10px", color: "var(--text2)", fontSize: 11 }}>{r.programme || "—"}</td>
                    <td style={{ padding: "9px 10px", color: "var(--text2)", fontSize: 11 }}>{r.timeBelt || "—"}</td>
                    <td style={{ padding: "9px 10px", fontSize: 11 }}><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}><Badge color="blue">{r.mediaType || "—"}</Badge>{isArchived(r) && <Badge color="red">Archived</Badge>}</div></td>
                    <td style={{ padding: "9px 10px", color: "var(--text2)", fontSize: 11 }}>{r.duration}"</td>
                    <td style={{ padding: "9px 10px", fontWeight: 600, color: "var(--accent)", fontSize: 11 }}>{fmtN(r.ratePerSpot)}</td>
                    <td style={{ padding: "9px 10px", color: "var(--red)", fontSize: 11 }}>{r.discount || 0}%</td>
                    <td style={{ padding: "9px 10px", color: "var(--red)", fontSize: 11 }}>{r.commission || 0}%</td>
                    <td style={{ padding: "9px 10px", fontWeight: 700, color: "var(--green)", fontSize: 11 }}>{fmtN(net)}</td>
...
const ensureScriptLoaded = (src, test) => new Promise((resolve, reject) => {
  try {
    ...
    script.src = src;
    script.async = true;
    script.onload = () => test() ? resolve() : reject(new Error(`Script loaded but global missing: ${src}`));
...
const ensureHtml2Canvas = async () => {
  await ensureScriptLoaded("https://cdn.jsdelivr.net/npm/html2canvas@1.4.1/dist/html2canvas.min.js", () => typeof window !== "undefined" && !!window.html2canvas);
  return window.html2canvas;
};

const ensureJsPdf = async () => {
  await ensureScriptLoaded("https://cdn.jsdelivr.net/npm/jspdf@2.5.1/dist/jspdf.umd.min.js", () => typeof window !== "undefined" && !!window.jspdf?.jsPDF);
...
const PrintPreview = ({ html, csv, pdfBytes, title, onClose }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const previewRoot = iframeDoc?.body;
      if (!previewRoot) throw new Error("Preview is not ready.");
      ...
      if (images.length) {
        await Promise.allSettled(images.map(img => img.complete ? Promise.resolve() : new Promise(res => {
      ...
      const canvas = await html2canvas(previewRoot, {
        ...
        allowTaint: true,
        logging: false,
        windowWidth: Math.max(previewRoot.scrollWidth, iframe.clientWidth || 0, 1120),
        windowHeight: previewRoot.scrollHeight,
        width: Math.max(previewRoot.scrollWidth, 1120),
        height: previewRoot.scrollHeight,
        scrollX: 0,
        scrollY: 0,
      });
      ...
      while (rendered < canvas.height) {
        const sliceHeight = Math.min(pageSlicePx, canvas.height - rendered);
      ...
      console.error("Live preview PDF export failed, falling back to built-in PDF:", err);
      if (!pdfBytes) {
        alert(err?.message || "Failed to generate PDF from preview.");
        return;
      }
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:2000, background:"rgba(0,0,0,.88)", display:"flex", flexDirection:"column", backdropFilter:"blur(6px)" }}>
      <div style={{ background:"#0e1118", borderBottom:"1px solid rgba(255,255,255,.1)", padding:"11px 18px", display:"flex", alignItems:"center", gap:10, flexShrink:0, flexWrap:"wrap" }}>
        ...
        {tab === "preview" && (<>
          {(pdfBytes || tab === "preview") && <button onClick={handleDownloadPDF} style={btnStyle("rgba(34,197,94,.15)","#22c55e","rgba(34,197,94,.35)")}>{preparingPdf ? "Preparing PDF..." : "⬇ Download PDF"}</button>}
      ...
      {visibleMpos.length === 0 ? <Card><Empty icon="📄" title="No MPOs yet" sub="Create your first Media Purchase Order" /></Card> :
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[...visibleMpos].sort((a, b) => b.createdAt - a.createdAt).map(m => (
            <Card key={m.id} hoverable style={{ padding: "14px 18px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
...
export default function App() {
  ...
  const [theme, setTheme] = useState(() => getDefaultTheme(authUser?.id || null));
  ...
  const toggleTheme = () => setTheme(t => {
    ...
    store.set(getThemeStorageKey(user?.id || authUser?.id || null), n);
    return n;
  });

  useEffect(() => {
    const nextTheme = getDefaultTheme(user?.id || authUser?.id || null);
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      try {
        ...
        if (active) {
          const recoveredAgencyId = await resolveAgencyIdForCurrentUser({ id: authUser.id, email: authUser.email, agencyId: authUser.user_metadata?.agency_id || null });
          let recoveredAgency = authUser.user_metadata?.agency_name || "My Agency";
          let recoveredAgencyCode = authUser.user_metadata?.agency_code || "";
          let recoveredAgencyAddress = "";

          if (recoveredAgencyId) {
            const { data: fallbackAgency } = await supabase
              .from("agencies")
              .select("name, agency_code, address")
              .eq("id", recoveredAgencyId)
              .maybeSingle();

            recoveredAgency = fallbackAgency?.name || recoveredAgency;
            recoveredAgencyCode = fallbackAgency?.agency_code || recoveredAgencyCode;
            recoveredAgencyAddress = fallbackAgency?.address || "";
          }
          ...
          setUser({
            ...
            agency: recoveredAgency,
            agencyId: recoveredAgencyId,
            agencyCode: recoveredAgencyCode,
            agencyAddress: recoveredAgencyAddress,
            role: normalizeRole(authUser.user_metadata?.role || "admin"),
            roleLabel: formatRoleLabel(authUser.user_metadata?.role || "admin"),
          });
        }
      }
    };
