...
const buildMPOPdf = (mpo) => {
  ...
  const fmtN = formatNumberFixed2;
  const clip = (s,n) => String(s||'').slice(0,n);
  const MN = MONTH_NAMES_SHORT;
  const FULL = MONTH_NAMES_FULL;
  const DAYS = DAY_LABELS_SHORT;
...
const buildMPOHTML = (mpo) => {
  ...
  const fmt = formatNumberFixed2;


  const MN = MONTH_NAMES_SHORT;
  const FULL = MONTH_NAMES_FULL;
  const DAY = DAY_LABELS_SHORT;
          ...
          {section === "security" && (
            <div className="fade">
              <Card style={{ marginBottom: 16 }}>
                ...
                <InfoRow label="Account Created" value={user.createdAt ? formatDateOnly(user.createdAt, "en-NG", { day: "numeric", month: "long", year: "numeric" }) : "—"} />
...
const ReportsPage = ({ vendors, clients, campaigns, rates, mpos }) => {
  ...
  const financeTracker = filteredMpos.map(mpo => ({
    ...
    paymentStatus: mpo.paymentStatus || "unpaid",
    paymentReference: mpo.paymentReference || "—",
    paidAt: mpo.paidAt ? formatDateOnly(mpo.paidAt) : "—",
  ...
  const buildSectionExport = (title, headers, rows, descriptor = "") => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${title}</title>
      ...
      <h1>${title}</h1>
      <p>Generated ${formatDateTime(new Date())}${descriptor ? ` · ${descriptor}` : ""}</p>
